...
const ItemFeed = ({ setSelectedCategory, selectedCategory }) => {
	...
	const { data, isLoading, error } = useFetch(`http://localhost:3500/menu/customerMenu/${cafe_id}`);
	console.log(data);

	// Filter items that are available and match the selected category
	const availableItems = data.filter((item) => item.is_available && (selectedCategory === null || selectedCategory === "All" || item.categories === selectedCategory));
			...
			{availableItems.length > 0 ? (
				<div className="flex justify-center items-start">
					...
					<div className="flex flex-col justify-start items-start text-xl p-6">
						...
						{cart.map(
							(cartItem) =>
								cartItem.quantity > 0 && ( // Only render if quantity is greater than 0
									<div key={cartItem.item_id} className="flex flex-col justify-start items-start">
										<p>{cartItem.item_name}</p>
										<p>Quantity: {cartItem.quantity}</p>
										{/* Quantity adjustment buttons */}
										<div className="">
											<button className="mr-2 text-center w-8 h-8 font-medium text-white hover:bg-green-600 transition duration-300 ease-in-out delay-60 bg-slate-600 rounded-3xl drop-shadow-lg" onClick={() => increaseQuantity(cartItem)}>
												+
											</button>
											<button className="text-center w-8 h-8 font-medium text-white hover:bg-red-600 transition duration-300 ease-in-out delay-60 bg-slate-600 rounded-3xl drop-shadow-lg" onClick={() => decreaseQuantity(cartItem)}>
												-
											</button>
										</div>
									</div>
						...
						<button className="px-5 py-2 mt-4 font-medium text-zinc-900 font-Rubik transition duration-300 ease-in-out delay-60 hover:-translate-y-1 hover:scale-105 bg-[#6859ea] hover:bg-gradient-to-r from-[#6859ea] to-[#6acbe0] rounded-3xl drop-shadow-lg" onClick={confirmOrder}>
							Confirm Order
						</button>
					</div>
				</div>
